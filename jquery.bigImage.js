...
;(function ($) {
    ...
    */
    ...
    function setStyles($anchor) {
        ...
        $zoomMask.css({
            overflow: 'hidden',
            width: Math.ceil(settings.zoom.width) + 'px',
            height: Math.ceil(settings.zoom.height) + 'px',
            position: 'absolute',
    ...
    function calculateImageRatios($smallImg, $largeImg, callback) {
        ...
        preload(smallImageUrl, largeImageUrl, function() {
            var ratios = {
            // Don't use jQuery width/height; WebKit returns 0 with jQ even
            // after load event fired and .complete state is true.
                height: $smallImg.height() / $largeImg[0].height,
                width: $smallImg.width() / $largeImg[0].width
            };
    ...
    function setupLens($lens, $smallImg, $largeImg) {
        calculateImageRatios($smallImg, $largeImg, function(ratios) {
            var $anchor = $smallImg.closest('a'),
                ...
                lensDimensions = {
                    height: Math.ceil(ratios.height * settings.zoom.height),
                    width: Math.ceil(ratios.width * settings.zoom.width)
                };
    ...
    // Turn off ALL zooms; zoom mask stays on page if active while new image is ajax loaded
    function turnOffZoom($anchor) {
        $('.zoom-mask').css({ left: '-9999px' });
        getLens($anchor).hide();
    }
    ...
    function moveZoom($lens, $smallImg, $largeImg, position) {
        calculateImageRatios($smallImg, $largeImg, function(ratios) {
            var mouseOffset = $smallImg.offset(),
                ...
                maxLensTop  = $smallImg.height() - lensHeight + iePlus4(),
                maxLensLeft = $smallImg.width() - lensWidth + iePlus4();
            ...
            $lens.css({
                top: Math.ceil(lensTop) + 'px',
                left: Math.ceil(lensLeft) + 'px'
            });
            ...
            $largeImg.css({
                top: (0 - Math.ceil(imgTop)) + 'px',
                left: (0 - Math.ceil(imgLeft)) + 'px'
            });
        });
    }
    ...
    */
    ...
    function iePlus4() {
        return $.browser.msie && ~~$.browser.version <= 7 ? 4 : 0
    }
